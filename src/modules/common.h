...
#define S21_IS_INF(x) \
  (x == S21_INF || x == -S21_INF)  ///< Check value is infinity (inf).
#define GET_SHIFT(x) \
  (sizeof(x) * 8 - 1);  ///< Returns the number of bits of the specified type
...
typedef union {
  union ieee754_double
      dbl;         ///< Union containing the double in question with fields.
  uint64_t ulong;  ///< A commensurate int having the same bit composition as
                   ///< the double.
} double_int;
